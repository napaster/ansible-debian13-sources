...
Штатный модуль умеет то же самое, но требует на хосте `python3-debian`. У нас
дисциплина «сначала `--check --diff`, потом прогон», и роль, которая на чистом
хосте не показывает дифф, эту дисциплину ломает. Шаблон не требует на целевом
хосте ничего, показывает файл целиком в диффе и одинаково ведёт себя в обоих
режимах. Плата за это — валидация полей на нашей стороне, она в
`tasks/main.yml` первым шагом.

Та же логика про `python3-apt` и модуль `apt`, но с неприятным поворотом:
начиная с ansible-core 2.19 у модуля `apt` есть `auto_install_module_deps`
(по умолчанию включён), и **он доставит `python3-apt` даже в режиме
`--check`** — сам прогон при этом всё равно упадёт с «python3-apt must be
installed to use check mode», а пакет на хосте уже появится. Проверено
22.09.2026 на pusk.fire.loc: dry-run поставил `python3-apt` и четыре
зависимости. Поэтому задача `Ensure python3-apt` в этой роли ставит его
явно и осознанно, а не как побочный эффект чужой проверки.

В режиме `--check` эта задача показывает `ok`, а не `changed`: модуль
`command` с `creates` не умеет отличать «файла нет, но я бы поставил» от
«всё на месте». На реальном прогоне пакет ставится.
